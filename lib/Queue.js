...
Queue.prototype = {
    ...
    next: function () {
        ...
        if (this.repeat != 1) {
            //If it isn't repeating tracks remove the current head of the queue and add it to the tail of the history
            this.played.push(this.tracks.shift());
        }
        //If repeat is set to repeat the whole queue and the queue is now empty
        if (this.repeat == 2 && this.tracks.length == 0) {
            //Set the tracks to be the history
            this.tracks = this.played;
            //Reset the history to be an empty array
            this.played = [];
        }
    ...
    sortByVotes: function () {
        ...
        this.tracks.forEach(function (track) {
            track.sortBy = this.shuffled ? track.votes : ((Math.random() - 0.5) * 1.3 + track.votes);
        });
     ...
     * @returns {{shuffled: *, repeat: *, tracks: (Array|*)}}
     */
    getJson: function () {
        return {
    ...
    /**
     * Set the current value for repeat
     * 0: no repeat
     * 1: repeat the current track
     * 2: repeat the whole queue
     * @param repeat
     */
            ...
            fs.readFile("./queue.json", function (err, data) {
                if (err) {
                    ...
                    try {
                        ...
                        me.shuffled = false;
                        me.add(data.tracks).then(function () {
                            //Set the repeat and shuffle values to the loaded values
    ...
    save: function () {
        ...
        return new Promise(function (resolve, reject) {
            ...
            var data = me.getJson();
            //Just save the link from the tracks
            //TODO save the votes too
