...
$(function () {
    ...
    async.series([loadTemplates, loadPages], function () {
        ...
        registerContextMenu(".track-table tr", function () {
...
function handleAction(action, uri, data, e) {
    ...
    if (action == "add") {
        ...
        if ("object" === typeof this) {
            var $div = $("<div>");
            var color = $(this).find(".text-content").data("bgColor")
            $div.attr("style", function (s) {
                return (!s ? "" : s) + " background-color:" + color + " !important;"
            })

            $div.addClass("pill-clicked").css({
                top: $(this).offset().top,
            ...
            $("body").append($div);
            $(this).css("animation-delay", "-1000ms");
            setTimeout(function () {
                $div.css({
                    width: $("main").width(),
                    height: 350,
                ...
                prom.then(function () {
                    $div.css({
                        height: $("main .view-detail-top").outerHeight()
                    })
        ...
        function addContent() {
            ...
            $("main .pill").each(function () {
                promises.push(getAverageColor($(this).find("img").attr("src")).then(function (color) {
                    ...
                    $(this).find(".text-content").attr("style", function (s) {
                        return (!s ? "" : s) + " background-color: rgb(" + color.join() + ") !important;"
                    }).toggleClass("dark-text", darkText).data("bgColor", "rgb(" + color.join() + ")")
                }.bind(this)))
            })
            $("main .view-detail-top").each(function () {
                console.log($(this).find("img.detail-image").attr("src"));
                promises.push(getAverageColor($(this).find("img.detail-image").attr("src").replace("https://u.scdn.co/images/pl/default/", "https://i.scdn.co/image/")).then(function (color) {
                    var darkText = (1 - ( 0.299 * color[0] + 0.587 * color[1] + 0.114 * color[2]) / 255) < 0.5;
            ...
            Promise.all(promises).then(function () {
                if ("function" === typeof page.after) page.after.call({}, $page);
...
function registerPage(name, fn, template, after) {
    pages[name] = {
        name: name,
        fn: fn,
        template: template,
