...
$(function () {
    ...
    async.series([loadTemplates, loadPages], function () {
        ...
        $("body").on("click", ".show-search-btn", function () {
        ...
        }).on("keypress", ".search", _.debounce(function () {
            doSearch($(this).val());
            ...
            $("header").removeClass("show-search");
            $(".search-content").removeClass("show");
            handleAction($(this).attr("data-action"), $(this).attr("data-uri"), $(this).parents("table").attr("data-uri"), e)
            ...
            return false;
        }).on("click", "aside.play-queue .vote-buttons button", function () {
...
function navigateTo(_page, args, noPush) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        setTimeout(function () {
            ...
            if (loaded) addContent(); else $("main").html(templates.pageLoader());
        }, 500);
...
function radiateDelayFrom(x, y) {
    ...
    $(".radiate-delay").each(function () {
        ...
        var delay = this.delay = Math.pow(Math.pow(pos.left - x, 2) + Math.pow(pos.top - y, 2), 0.5) / 3;
        min = Math.min(delay, min);
    });
    console.log(min);
    $(".radiate-delay").each(function () {
        $(this).css("animation-delay", this.delay - min + "ms");
    });
}