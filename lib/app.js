...
var Queue = new require("./Queue"),
	...
	traceFile: config.spotify.traceFileLocation
    }),
...
var queue, player, normalVolume, phoneVolume = 75, isPhone = false, lastTime, accessToken, accessTokenTimeout, accessTokenExpireTime = 0;
...
function updateAccessToken() {
    ...
    if (accessTokenExpireTime > (new Date).getTime() + 60000) {
        //More than 60 secs in future
        accessTokenTimeout = setTimeout(updateAccessToken, accessTokenExpireTime - (new Date).getTime() - 30000);
        return;
    }
    ...
    request({
        ...
        accessTokenExpireTime = (new Date).getTime() + (response.expires_in * 1000);
