...
var queue, prePhoneVolume, phoneVolume = 75, lastTime, accessToken, accessTokenTimeout;
...
spotify.on({
    ready: function () {
        ...
        io.on('connection', function (socket) {
            ...
            socket.on("addPlaylist", function (playlistURI) {
                ...
                if (playlist.isLoaded) {
                    queue.addTracks(playlist.getTracks());
                } else {
            ...
            socket.on("setPhoneVolume", function (volume) {
                phoneVolume = parseInt(volume);
            })
            ...
            socket.on("vote", function (data) {
                ...
                queue.vote(data.uuid, data.up ? true : false, socket.request.connection.remoteAddress);
            })
            socket.emit("ip", socket.request.connection.remoteAddress);
            socket.emit("playState", queue.isPlaying)
            socket.emit("accessToken", accessToken);
            queue.changedCurrent();
            socket.emit("seek", lastTime);
            loudness.getVolume(function (err, volume) {
                socket.emit("volume", volume);
            });
            if (queue.getTracks().length > 60) {
                setTimeout(function () {
                    socket.emit("playQueue", queue.getJson()) //fix for stupid socket.io crash
                }, 250);
            } else {
                socket.emit("playQueue", queue.getJson()) //fix for stupid socket.io crash
            }
        ...
        queue.on("changedCurrent", function (track) {
            io.emit("changedCurrent", track);
        })
        queue.on("playState", function (isPlaying) {
            io.emit("playState", isPlaying);
        })
        queue.on("playError", function (error) {
