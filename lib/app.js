...
spotify.on({
    ready: function () {
        ...
        console.log("Started on port %s:%i", config.http.ip, config.http.port);
...
function updateAccessToken() {
    ...
    var client = request({
        ...
        accessTokenTimeout = setTimeout(updateAccessToken, response.expires_in - 60); //Update 60 seconds before this one expires (about 59 minutes)
...
spotify.login(config.spotify.username, config.spotify.username)
updateAccessToken();
