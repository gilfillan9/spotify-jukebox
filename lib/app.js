                    ...
                    fs.writeFile(config.spotify.authDir + ".selected", file, function (err) {
                        if (err) {
                            ...
                            spotify.logout(function () {
                                var data = JSON.parse(fs.readFileSync(config.spotify.authDir + authFiles[authFiles.indexOf(file)]))
                                spotify.on({
                                    ready: function () {
                                        console.log("Logged in");
                                        player.play();
...
var selectedFile = fs.readFileSync(config.spotify.authDir + ".selected")
...
if (authFiles.length == 0) {
    ...
    var data = JSON.parse(fs.readFileSync(config.spotify.authDir + selectedFile))
    spotify.login(data.username, data.password)
}
